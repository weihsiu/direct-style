...
object SocksProxy:
  ...
  val requestCodec = discriminated[Request]
    .by(uint16)
    .subcaseP[Request.ConnectRequest](0x0401) { case x: Request.ConnectRequest => x }(
  ...
  val replyCodec = (constant[Byte](0) :: replyCodeCodec :: uint16 :: bytes(4)).as[Reply]
  ...
  def handleRequest(inSocket: Socket)(using Ox): Unit =
    ...
    def tunnel(socket1: Socket, socket2: Socket)(using Ox): Unit =
      def forward(socket: Socket, in: BufferedInputStream, out: BufferedOutputStream): Unit =
    ...
    match
      case Request.ConnectRequest(destPort, destIp, userId) =>
        ...
        val outSocket = Socket(InetAddress.getByAddress(destIp.toArray), destPort)
