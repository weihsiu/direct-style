...
abstract class Conduit[A](processFunc: (ArrayBuffer[Source[A]], ArrayBuffer[Sink[A]]) => Unit)(using
    Ox
) extends Closeable:
  private class ConduitActor(processFunc: (ArrayBuffer[Source[A]], ArrayBuffer[Sink[A]]) => Unit):
    var senders: ArrayBuffer[Source[A]] = ArrayBuffer.empty
    var receivers: ArrayBuffer[Sink[A]] = ArrayBuffer.empty
    def startProcess() = forkCancellable(forever(processFunc(senders, receivers)))
  ...
  def addReceiver(receiver: Sink[A]): Unit = conduitActor.ask(_.addReceiver(receiver))
  def removeSender(sender: Source[A]): Unit = conduitActor.ask(_.removeSender(sender))
  def removeReceiver(receiver: Sink[A]): Unit = conduitActor.ask(_.removeReceiver(receiver))
