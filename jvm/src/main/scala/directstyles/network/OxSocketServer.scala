...
trait OxSocketServer:
  def sockets(port: Int, capacity: Int = 1000)(using Ox): (Source[Socket], () => Unit)
  def channel(socket: Socket, capacity: Int = 10)(using IO, Ox): Channel[Chunk[Byte]]
...
object OxSocketServer:
  def apply(): OxSocketServer = new OxSocketServer:
    ...
    def channel(socket: Socket, capacity: Int = 10)(using IO, Ox): Channel[Chunk[Byte]] =
      // kludge
      val channel =
        Source.fromInputStream(socket.getInputStream()).asInstanceOf[Channel[Chunk[Byte]]]
