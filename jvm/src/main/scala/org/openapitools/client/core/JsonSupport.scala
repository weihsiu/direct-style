...
object JsonSupport extends SttpJson4sApi {
  def enumSerializers: Seq[Serializer[?]] = Seq[Serializer[?]]() :+
    new EnumNameSerializer(IncludeEnum)

  private class EnumNameSerializer[E <: Enumeration: ClassTag](enumeration: E)
    ...
    private def isValid(json: JValue) = json match {
  ...
  implicit val format: Formats =
    DefaultFormats ++ enumSerializers ++ DateSerializers.all ++ AdditionalTypeSerializers.all
